    ...
    *,
    ...
    if task == "vision":
        ...
        if client is None:
            ...
            # For auto/custom, fall back to auto-detection chain (not OpenRouter)
            if not resolved_base_url:
                logger.info("Auxiliary %s: provider %s unavailable, falling back to auto-detection",
                            task or "call", resolved_provider)
                client, final_model = _get_cached_client(
                    "auto", resolved_model or _OPENROUTER_MODEL)
        if client is None:
            raise RuntimeError(
                f"No LLM provider configured for task={task} provider={resolved_provider}. "
                f"Run: hermes setup")
    ...
    *,
    ...
    if task == "vision":
        ...
        if client is None:
            ...
            if not resolved_base_url:
                logger.warning("Provider %s unavailable, falling back to auto-detection",
                               resolved_provider)
                client, final_model = _get_cached_client(
                    "auto", resolved_model or _OPENROUTER_MODEL,
                    async_mode=True)
